...
function kameraBaslat() {
    ...
    const un = db.collection('kameralar').onSnapshot(s => {
...
function renderKameraListesi() {
    ...
    const list = kameralar
        .filter(k => !q || trLower(k.adres + ' ' + k.ip + ' ' + (k.aciklama || '')).indexOf(q) > -1)
        .sort((a, b) => a.adres.localeCompare(b.adres, 'tr'));

    let html = list.map(k =>
        `<div class="kamera-row flex items-center gap-2">
...
function openKameraAddModal() {
    openModal('Kamera Ekle', kameraFormAlanlari() + '<button class="btn btn-blue w-full mt-3" onclick="kameraKaydet()">Kaydet</button>');
}
...
async function kameraKaydet() {
    const d = kameraFormOku();
    if (!d.adres) { toast('Adres girin'); return; }
    await db.collection('kameralar').add(d);
    closeModal();
    toast('Kamera eklendi');
}

function kameraDuzenleModal(id) {
    const k = kameralar.find(x => x.id === id);
    if (!k) return;
    openModal('Kamera Düzenle', kameraFormAlanlari(k) + `<button class="btn btn-blue w-full mt-3" onclick="kameraGuncelle('${id}')">Kaydet</button>`);
}

async function kameraGuncelle(id) {
    const d = kameraFormOku();
    if (!d.adres) { toast('Adres girin'); return; }
    await db.collection('kameralar').doc(id).set(d, { merge: true });
    closeModal();
    toast('Güncellendi');
}

function kameraSil(id) {
    appConfirm('Bu kamerayı silmek istediğinizden emin misiniz?', async () => {
        await db.collection('kameralar').doc(id).delete();
        toast('Silindi');
    }, 'Kamerayı Sil');
}
