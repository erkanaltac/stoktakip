// ariza.js – Arıza Modülü (formatText eklendi)
let arBolum = 'alt';
...
function arizaListeCiz(b, elId){
  ...
  const list = arKayitlar.filter(a=>a.bolum=== (b==='ust'?'E5 Üstü':'E5 Altı') && a.durum!=='Tamamlandı').sort((a,b2)=>b2.tarihISO.localeCompare(a.tarihISO));
  var html = list.map(a=>{
...
function arizaDuzenle(id){
  ...
  openModal('Arıza Düzenle', `<div class="grid3">
    ...
    <div style="grid-column:1/-1"><label class="f">Parçalar</label><textarea id="ea2-parcalar" style="min-height:90px">${esc(arParcaAdlari(a).split(', ').join('\n'))}</textarea></div>
    <div style="grid-column:1/-1"><label class="f">Açıklama</label><input id="ea2-aciklama" value="${esc(a.aciklama||'')}"></div>
    <div><button class="btn btn-blue w-full mt-4" onclick="arizaGuncelle('${id}')">Güncelle</button></div>
  </div>`);
}
...
async function arizaGuncelle(id){
  ...
  const yeniAdlar = document.getElementById('ea2-parcalar').value.split(/[\r\n]+/).map(s=>formatText(s)).filter(Boolean);
  if(yeniAdlar.length===0){ toast('En az bir parça girin'); return; }
  const a = arizaBul(id);
  const yeniParcalar = yeniAdlar.map(ad=>{ const eski=(a.parcalar||[]).find(p=>trLower(p.ad)===trLower(ad)); return { ad, adet: eski?eski.adet:1, tamam: eski?eski.tamam:false }; });
  const tumuTamam = yeniParcalar.every(p=>p.tamam);
  const update = { adres, genelAdet:Number(document.getElementById('ea2-adet').value)||1, hedefTarih:document.getElementById('ea2-hedef').value, aciklama, parcalar:yeniParcalar, durum: tumuTamam?'Tamamlandı':'Açık' };
  await db.collection('ariza_kayitlari').doc(id).set(update, {merge:true});
  closeModal(); toast('Güncellendi');
}

function arizaSil(id){ appConfirm('Bu arıza kaydını silmek istediğinizden emin misiniz?', async ()=>{ await db.collection('ariza_kayitlari').doc(id).delete(); toast('Kayıt silindi'); }); }
